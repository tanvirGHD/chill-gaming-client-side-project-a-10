  ...
  const handelUserDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`https://assignment-10-server-side-neon.vercel.app/review/${id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
            ...
            <tbody>
              {reviews.map((review, index) => (
                <tr key={review._id}>
                  ...
                  <td className="text-left">{review.gameTitle}</td>
                  <td className="text-left">{review.reviewDescription}</td>
                  <td>{review.rating}</td>
                  <td>
                    ...
                    <button className="action-button" onClick={() => handelUserDelete(review._id)}>
                      <img
                        className="h-10 w-10"
